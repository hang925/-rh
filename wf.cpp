...
void wordGroups(char *fileName, int n){	//说明要输出多少个词的短语，并按照出现频率排列。同一频率的词组， 按照字典序来排列
	start = clock(); 
	ifstream inFile;
	inFile.open(fileName);
	if( !inFile.is_open()){
...
vector<string> getFileList(string dir)
{
	...
	do{
		if (findData.attrib & _A_SUBDIR){//检索子目录问题 
			if (strcmp(findData.name, ".") == 0 || strcmp(findData.name, "..") == 0)//若该目录是"."或".."，则进行下一次循环，否则输出子目录名，并进入下一次搜索
				continue;
			string dirNew = dir + "\\" + findData.name;//在目录后边添加"\\"和搜索到的目录名进行下一次搜索
...
vector<string> getFileListInMenu(string dir){//遍历指定文件目录，但是会遍历目录下的所有子目录，对目录下每个文件执行wf.exe -f <file>
	vector<string> allPath;
	string dir2 = dir + "\\*.*";//进行第一次搜索
	intptr_t handle;
	_finddata_t findData;
...
void wordStatisticsTopK(char *fileName, const int &k)   ////支持 -n 参数，输出出现次数最多的前 n 个单词。
{
	start = clock(); 
	ifstream inFile;
	inFile.open(fileName);
...
void dynamicPrototype(char *fileName1,char *fileName2){ //支持动词形态的归一化，参数为 -v
	start = clock(); 
	ifstream inFile1;
